...
int main(int argc, char* argv[]) {
  ...
  program.add_argument("-p", "--pose")
      .help(
          ...
          "\033[32msix\033[0m\n"
          "\033[34m\texample_airbot_ro_hand_pose -p six\033[0m\n"
  ...
  HandState six_pose;
  six_pose.positions = {0, 0, 1000, 1000, 1000, 0};
  std::fill(six_pose.forces.begin(), six_pose.forces.end(), 0);
  std::fill(six_pose.velocities.begin(), six_pose.velocities.end(), 0);
  pose_presets["six"] = six_pose;
  ...
  if (pose_presets.find(pose_name) == pose_presets.end()) {
    ...
    std::cerr << "Available poses: shake, six, praise" << std::endl;
    return 1;
  }
  size_t pose_idx = 0;
  ...
  while (running) {
    ...
    if (ch == ' ') {
      ...
      if (pose_idx == 1) {
        selected_pose = pose_presets["six"];
